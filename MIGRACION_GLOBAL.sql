...
CREATE POLICY "movimiento_insert_ops"        ON movimiento FOR INSERT TO authenticated WITH CHECK (get_my_role() IN ('superadmin', 'operador') OR (get_my_role() = 'cajero' AND tipo = 'DESPACHO') OR (get_my_role() = 'economico' AND tipo = 'DEPOSITO'));
...
CREATE POLICY "movimiento_delete_superadmin" ON movimiento FOR DELETE TO authenticated USING (get_my_role() = 'superadmin' OR (get_my_role() = 'cajero' AND tipo = 'DESPACHO'));
...
BEGIN
  ...
  -- porque los errores de tarifa se detectan tras entregar o cobrar.
  -- Ver migrations/2026-08-14_precio_editable_cualquier_estado.sql
...
-- movimiento: validar stock en alta, edicion y borrado
-- Ver migrations/2026-08-13_validar_stock_update_delete.sql
-- fn_stock_disponible replica la formula de v_stock_tanques (incluida la regla
-- de surtidores y el filtro por combustible). p_excluir ignora un movimiento,
-- necesario al validar un UPDATE porque la fila vieja sigue en la tabla.
...
-- Aviso al operador en el momento del despacho (INSERT y UPDATE)
...
-- Red de seguridad: ningun deposito ni surtidor puede quedar en negativo.
-- AFTER, para comprobar el estado final ya aplicado en lugar de simularlo.
...
-- ─────────────────────────────────────────────────────────────
--  CPP ajuste manual + vistas corregidas (2026-06-22)
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
--  2026-07-17: tarjetas_vinculadas_ids (array) en surtidores
-- ─────────────────────────────────────────────────────────────
-- Ver migrations/2026-07-17_tarjetas_vinculadas_array.sql
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Publicacion de versiones de la APK (2026-09-10)
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Tiempo real: publicacion de cambios (2026-09-14)
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Adjuntos de movimiento: columnas, almacen y permisos (2026-09-14)
-- ═══════════════════════════════════════════════════════════════════════════
